...
if ~stoprunnning
    ...
    colorspace = questdlg('Color scale?','','RGB','Gray','Gray');
    ...
    switch colorspace
        ...
        case 'RGB'
            [v,expRate_OF] = setsettingsvid(v,'ok');      % logging/Color/Trigger
    end
...
if ~stoprunnning
    ...
    DT(DT==':')='_';
    % Sufix:
    prompt = {'Enter suffix for the video recording (mouse ID, #Box, etc)'};
    dlgtitle = 'Mouse ID';
    definput = {'Mouse-0'};
    dims = [1 40];
    ...
    fprintf('\n end.\n > Cleaning:')
    Stop_OpenField_SingleREC;

    fprintf('<a href="matlab:dos(''explorer.exe /e, %s, &'')">Recorded Videos Here</a>\n',SF)
